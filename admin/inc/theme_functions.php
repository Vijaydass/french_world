	...
	if (count($pagesSorted) != 0) { 
		foreach ($pagesSorted as $page) {
			...
			if ($page['menuStatus'] == 'Y') { 
				...
				$menu .= '<li class="rd-nav-item '. $classes .'"><a href="'. find_url($page['url'],$page['parent']) . '" class="rd-nav-link" title="'. encode_quotes(cl($page['title'])) .'">'.strip_decode($page['menu']).'</a></li>'."\n";
			}
		}
